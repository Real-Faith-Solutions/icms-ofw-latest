...
//        }
        ...
        switch ((int) $aParam['tab']) {
            case 1:
                ...
                // For Agency branch - ALL CASES 
                    ...
                    $aParam['cTab'] = 'AND `ic`.`case_added_by` IN (SELECT `user_id` FROM `icms_user` WHERE `agency_branch_id` = ' . $_SESSION['userData']['agency_branch_id'] . ' )';
                }
            ...
            case 2:
                ...
                break;
            case 3:
                $aParam['cTab'] = 'AND `ic`.`case_id` IN (SELECT `icv`.`case_id` FROM `icms_case_victim` `icv` WHERE `icv`.`case_victim_id` IN ( SELECT `case_victim_id` FROM `icms_case_victim_services` `icvs` WHERE `icvs`.`case_victim_services_is_active` = "1" AND  `icvs`.`case_victim_services_id` IN ( (SELECT `icvsa`.`case_victim_services_id` FROM `icms_case_victim_services_agency_tag` `icvsa` WHERE `icvsa`.`case_victim_services_agency_tag_is_active` = "1" AND `icvsa`.`agency_branch_id` = ' . $_SESSION['userData']['agency_branch_id'] . ' ) )  ) )';
                break;
            case 4:
                if ((isset($aParam['user_id'])) && ($aParam['user_id'] !== "" || empty($aParam['user_id']) == false)) {
                    $aParam['user_id'] = $this->yel->decrypt_param($aParam['user_id']);
                    $aParam['cTab'] = 'AND `ic`.`case_id` IN (SELECT `ictu`.`case_id` FROM `icms_case_tagged_users` `ictu` WHERE `ictu`.`user_id` = ' . $aParam['user_id'] . ' AND `ictu`.`case_tagged_users_status` = 1)';
                } else {
    ...
    public function __addCase($aParam) {
        ...
        if (empty($_SESSION['userData']) !== true) {
            ...
            $aResult['cheker'] = $this->Case_model->addCaseChecker(json_encode($aNewParam));
            $aResponse['checker_id'] = $this->yel->encrypt_param($aResult['cheker']['insert_id']);
...
//        }
    ...
    private function addCaseVictim($aNewParam) {
        ...
        if ($_SESSION['userData']['user_level_id'] !== "1") {
            ...
            //send system notification to branch admin
            $aNotif = [];
            $aNotif['method'] = "view_victim_services";
        ...
        $superAdmin = $this->Users_model->getSuperAdminUserId();
        ...
        $aNotif['method'] = "view_victim_services";
        $aNotif['tbl_id'] = $aNewParam['case_id'];
        $aNotif['receiver'] = $superAdmin['user_id'];
        $aNotif['notif_type'] = "1";
        $aNotif['msg'] = "You have successfully ";
        $aNotif['msg'] .= " created a new case report <orange><a href='update_case/" . $this->yel->encrypt_param($aNewParam['case_id']) . "'>" . $aNewParam['case_number'] . "</a></orange>";
        $this->notif->create($aNotif);
        //send email to iacat
        $aEmail = [];
        $aEmail['to'] = $superAdmin['user_email'];
        $aEmail['subject'] = 'New case added';
        $aEmail['message'] = 'Hi ' . $superAdmin['user_firstname'] . ',  <br><br> A new case has been added in the system with case number (' . $aNewParam['case_number'] . ') by ' . $agencyData['user_firstname'] . ' ' . $agencyData['user_lastname'] . ' of ' . $agencyData['agency_name'] . ' (' . $agencyData['agency_branch_name'] . ').  <br>';
        $aEmail['message'] .= '<br>To view details of the case, login to ICMS using your account and search using the case number.<br>';
        $aResponse['mail'] = $this->mailbox->sendMail($aEmail);
